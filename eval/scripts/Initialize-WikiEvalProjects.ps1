...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $repositoryRoot 'eval\graders\dev\wiki-auto-apply-v1.expected.json') -Destination (Join-Path $graderPath 'graders\dev')
  Copy-Item -LiteralPath (Join-Path $repositoryRoot 'eval\graders\dev\wiki-cleanup-v1.expected.json') -Destination (Join-Path $graderPath 'graders\dev')
