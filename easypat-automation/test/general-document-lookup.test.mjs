...
const documentEnvelope={templateId:"matter-detail.documents.matter-candidate.v1",command:"SELECT",statements:["SELECT GRP_KEY,DELETEFLG,DOC_NUM,DIV,DOC_NAME,REG_DATE,FILE_NAME,FILE_NAME_UPLOAD,FILE_SIZE FROM docs WHERE DELETEFLG='N' AND DOC_NUM=9 AND DIV='domestic' AND GRP_KEY='100'"]};
...
function setup({wrongGroup=false,count="1",emptyDocuments=false}={}){
  const calls=[];const lookup=createGeneralDocumentLookup({countDefinition,searchDefinition,documentDefinition,loadTemplate:async id=>structuredClone(templates.get(id)),executeRead:async request=>{
    ...
    return{templateId:documentEnvelope.templateId,columns:documentColumns,rows:emptyDocuments?[]:[{GRP_KEY:wrongGroup?"902":"901",DELETEFLG:"N",DOC_NUM:"9",DIV:"domestic",DOC_NAME:"수임서류",REG_DATE:"2026-09-16",FILE_NAME:"수임내역서.pdf",FILE_NAME_UPLOAD:"upload/app_proc/2026/09/16/verified-file.pdf",FILE_SIZE:"12345"}]};
  }});return{lookup,calls};
}
...
test("generic document lookup accepts an exact-schema empty result",async()=>{
  const {lookup,calls}=setup({emptyDocuments:true});const value=await lookup.list({matterReference:"PT261130"});
  assert.deepEqual(calls,["count-results","fetch-result-rows","document-records"]);assert.deepEqual(value,{matterReference:"PT261130",count:0,items:[]});
});
