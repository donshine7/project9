...
try {
    ...
    $message = "Fiddler의 기존 세션 9를 선택하세요.`n위쪽 Request → Body → Form-Data를 누르세요.`nKey가 sql인 행의 Value 셀을 한 번 클릭해 선택한 뒤 Ctrl+C를 한 번 누르세요.`nText 화면, 다른 셀, 전체 복사 아이콘은 사용하지 마세요.`nPowerShell 창에는 붙여넣지 마세요.`n`n복사 후 이 확인창의 확인을 누르세요."
    $null = [System.Windows.MessageBox]::Show($message, 'EasyPAT 사건별 문서 요청 검사', [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
    ...
    $requestStart.ArgumentList.Add((Join-Path $PSScriptRoot '..\src\capture-matter-document-request.mjs'))
    ...
    if ($requestSummary.serverRequestsPerformed -ne 0 -or $requestSummary.productionEnabled -ne $false -or
        $requestSummary.rawValuesReturned -ne $false -or $requestSummary.status -notmatch '^matter-document-request-(captured|rejected)$') { throw 'Summary rejected' }
    Write-Output ($requestSummary | ConvertTo-Json -Depth 6 -Compress)
    exit $requestProcess.ExitCode
} catch {
    Write-Output '{"status":"matter-document-request-helper-rejected","rawValuesReturned":false,"serverRequestsPerformed":0,"productionEnabled":false}'
    exit 1
} finally {
    $requestText = $null
